...
def BwareaOpen(img,MinArea):
    thresh = cv2.threshold(img, 0, 255, cv2.THRESH_BINARY)[1] # 이진화 수행, 흑백이미지 생성
    # Filter using contour area and remove small noise
    cnts = cv2.findContours(thresh, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)[1] # 모든 윤곽 찾기
    cnts_TooSmall = []
...
# 두 컨투어 간 최소 중심 거리 및 각 컨투어의 중심점 계산
...
# 두 컨투어간 최소 거리와 최적의 중간선 설정을 위한 거리 계산 및 중간 차선 추정
