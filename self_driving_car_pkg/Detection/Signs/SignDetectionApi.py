...
model_loaded = False
model = 0
sign_classes = ["speed_sign_30","speed_sign_60","speed_sign_90","stop","left_turn","right_turn","No_Sign"] # Trained CNN Classes
sign_flag = False
sign_delay_cnt = 0
...
def SignDetection_Nd_Tracking(gray,cimg,frame_draw,model):
    global sign_flag, sign_delay_cnt
    # 3. IF Mode of SignTrack is Detection , Proceed
    if (signTrack.mode == "Detection" and sign_flag == False): # 표지 인식이 활성화 되지 않은 경우 detection 수행
        # cv2.putText(frame_draw,"Sign Detected ==> "+str(signTrack.Tracked_class),(20,85),cv2.FONT_HERSHEY_COMPLEX,0.75,(255,255,0),2)
        NumOfVotesForCircle = 33 #parameter 1 MinVotes needed to be classified as circle
    ...
    # 5. IF Mode of SignTrack is Tracking , Proceed
        ...
        if sign_flag == False: # 표지 인식이 활성화 되지 않았을 때만 수행
            ...
            if ( (p1 is None) or ( len(p1[st == 1])<3 ) ):
                ...
                signTrack.mask = np.zeros_like(frame_draw)
                signTrack.Reset()
                sign_flag = True # tracking이 끝났을 때 표지 인식 활성화
    ...
    if sign_flag: # 표지 인식 활성화되면 delay 카운트
        sign_delay_cnt += 1
        print("detection delay", sign_delay_cnt)
    if sign_delay_cnt == 50: # delay 카운트가 50이 되면 초기화 및 표지 비활성화(표지 detection 재개)
        sign_flag = False
        sign_delay_cnt = 0
        print("delay finish")
...
def detect_Signs(frame,frame_draw):
    ...
    if not model_loaded:
        ...
        # 1. Load CNN model
        global model
        model = load_model(os.path.join(os.getcwd(),"/home/uu007007007/project_ws/src/self_driving_car_pkg/self_driving_car_pkg/data/data/saved_model_Ros2_5_Sign.h5"),compile=False)
